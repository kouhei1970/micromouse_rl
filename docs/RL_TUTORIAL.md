...
> ⚠️ **一部失効（2026-08-20 の棚卸し）**: §1〜6 の一般的な強化学習・SB3 の解説は有効。
> **§7 の実践例は凍結レガシー（`legacy/` の旧 Phase1〜3 構成）を前提としており失効している**
> （`README.md` が参照・流用を禁じている対象）。現行の構成は `mouse/` と `classic/` である。
...
#### 行動（Action）
**エージェントが選択する操作**です。ゲームで言えば「コントローラの入力」に相当します。
...
- **連続行動**: 実数値で指定（例：モータ電圧 2.5V）← 本プロジェクトはこちら
...
ステップ1:
  ...
  行動: 左モータ2.0V、右モータ2.0V（加速）
  結果: 速度が0.45m/sに上昇
  報酬: +0.9（目標に近づいた！）

ステップ2:
  観測: 前方の壁まで0.05m、目標速度0.5m/s、現在速度0.45m/s
  行動: 左モータ2.0V、右モータ2.0V（そのまま加速）
  結果: 壁に衝突！
  報酬: -10（失敗）
  → エピソード終了
...
モータ電圧という連続行動空間を扱い、シミュレーションで大量の経験を集められるため、安定性の高い**PPO**を採用しています。実機での学習など経験収集コストが高い場合は、SACやTD3がより適しています。
...
最も基本的なタスクです。目標速度を追従するモータ制御を学習します。
