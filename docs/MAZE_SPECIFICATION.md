...
> **本書の位置づけ（2026-08-12 教授注記）**: §1〜§3（座標系・迷路構成要素・初期配置）は
> 2026-08-12 の独立監査（`verification/AUDIT_004_maze_spec_consistency.md`）で現行実装
> （`mouse/params.py`・`mouse/mjcf.py`・生成迷路 XML）と**全項目一致**を確認済みで、有効である。
> **§4 は凍結レガシー（旧 Phase 2）の記述であり、現行の規約ではない**（§4 冒頭の警告を参照）。
> 競技プロトコル（計時・ゴール判定・持ち時間）の正本は `docs/RESEARCH_PLAN.md` §2。
...
> **機体中心での判定は正本（`docs/RESEARCH_PLAN.md` §2）が名指しで禁じている**。
> 本節を現行課題の仕様として読まないこと。
