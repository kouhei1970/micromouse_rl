...
def v2_final_section() -> None:
    ...
    print("=" * 92)
    LIMIT = 2000
    # 🔴 ΔT の引き直し（2026-08-14）: 旧 7.0 は **直進・全速進入の下界**であり典型値ではない
    # （`check_...:523` の初出が「3 面とも 7 歩」。深さ 56.9 mm ÷ 1 歩 9.6 mm = 5.93 → 6 歩）。
    # exp_019 の**方策実走**での実測は **n=43・中央値 15 歩・最小 6 歩・最大 283 歩**
    # （准教授の中間 n=23 と整合）。**余裕の見積もりに使うのは方策実走の側**である。
    dT_meas = 15.0           # 方策実走の中央値（exp_019・n=43）
    spc = CELL / (0.96 * DT)
    ...
    for seed in VALID_SEEDS:
        ...
        # 🔴 入力の引き直し（2026-08-14・exp_019 の v2 実走で測り直した）
        #
        # T_obs = **「エピソード内の平均衝突間隔」の中央値**（exp_019 の 6 seed・
        #   エピソード長 ÷ (リスポーン回数 + 1) を面ごとに出し、その中央値）。
        #   実測 = seed1 500.0 ／ seed2〜6 666.7 歩。**この量を採るのは、上限 2000 歩での
        #   衝突回数 N = LIMIT // T を実測（`n_respawn` 平均 2.50〜3.40）と合わせるため**である。
        #   ⚠️ **准教授の 186.5 歩は「個々の衝突間隔」の中央値**であり、**別の量**である
        #   （間隔の分布が右に歪むので、個々の中央値は平均間隔よりずっと小さくなる）。
        #   **どちらが正しいかではなく、模型のどの入力に要るかで選ぶ**（§9-17 の量の名指し）。
        # visit_obs = **割引後**の訪問の取り分。**AUDIT_030 の実測 0.0305**（旧 0.055 は 1.80 倍過大）。
        #   ⚠️ 学生B の 0.08〜0.10 は**割引なしの素の合計**で、AUDIT_030 の割引なし中央値 0.0800 と一致する。
        # alpha_obs = **v1 由来のまま（未確認）**。v2 の実測は取っていない。
        T_obs, alpha_obs, visit_obs = 666.7, 0.133, 0.0305
        phi_c = alpha_obs * d0 * CELL
    ...
    print(f"  ⚠️ 衝突シナリオは**実走 regime の実測値**"
          f"（T≈{T_obs:.0f} 歩・α≈{alpha_obs}・訪問 +{visit_obs}）を"
          "入力にした半実証の模型である\n"
          "     — (a') のときと違い**厳密な反実仮想は作れない**（軌道が変わるため）。")
    print("  🔴 **この模型は近似であり、正本ではない**（2026-08-14・AUDIT_030）:\n"
          "     入力を実測へ引き直しても、**構造そのものが実走と合っていない** —\n"
          "     実走の典型は**終盤に 1 回だけ衝突する**形で、**N = LIMIT // T という等間隔の\n"
          "     繰り返しは実走の姿ではない**。\n"
          "     **順序の正本は実走の直接測定（AUDIT_030）である**: 67 本の中央値で\n"
          "     **滞留 > 衝突（差 +0.0387）**。ただし**39% の走行では順序が崩れる**。\n"
          "     ⚠️ **したがって「18/20」も「0/20」も、確定した結論として引用しないこと。**")
    print("  🔴 **この節の出力は入力に極端に敏感である**（同じ式で 3 つの数が出た）:\n"
          "     ・**18/20** … 旧式（二重没収）＋ v1 入力（T=608・訪問 0.055）\n"
          "     ・**0/20**  … 是正式 ＋ v1 入力（AUDIT_025 の報告値）\n"
          "     ・**20/20** … 是正式 ＋ v2 実測入力（T=667・訪問 0.0305）← 本実行\n"
          "     **3 つとも同じ模型であり、違うのは入力だけである。**\n"
          "     ⇒ **「何/20」という指標そのものを結論に使わないこと。**\n"
          "        **正本は実走の直接測定（AUDIT_030）**であり、本節は機構を見るための道具に留める。")
    # 🔴 **なぜ使わないか**を 1 行で残す（准教授の提案・2026-08-14）。
    # これを書かないと、次の担当が同じ模型を作り直す。
    print("  🔴 **なぜ使わないか（次の担当がこの模型を作り直さないために）**:\n"
          "     **入力を実測へ替えるだけで 18/20 → 0/20 → 20/20 と動いた。**\n"
          "     **模型の構造（均等間隔の衝突）が実走（終盤に 1 回）と合っていないため、\n"
          "     入力を正しても代表値にならない。**\n"
          "     実測（AUDIT_030）: 衝突罰の割引後総和は **中央値 0.0003・平均 0.2009**、\n"
          "     模型（N=3・T=600）は **0.052** — **中央値の 170 倍・平均の 1/4** で、\n"
          "     **分布が右に強く歪むため模型の値は中央値でも平均でもない中間に落ちる**。\n"
          "     ⇒ **『典型的な走行』を表していない。**")
