...
def respawn_section(spc: float, dT: float) -> None:
    ...
    for alpha in (0.25, 0.5, 1.0):
        for n_crash in (1, 3, 10):
            for d0 in (4, 9, 15):
                ...
                # 各衝突で −1.0（罰）と −Φ_c（整形の巻き戻し）を γ^t で払う。
                crash_cost = sum((GAMMA ** (i * Tc)) * (1.0 + phi_c)
    ...
    for d0 in (4, 9, 15):
        ...
        # (c) リスポーン: 時間罰は 6000 歩ぶん全部・整形は取り返される・衝突罰は回数ぶん
        one = time_full - GAMMA ** (T_obs - 1) * (1.0 + phi_c) + visit_obs
        many = time_full + visit_obs - sum(
            GAMMA ** (i * T_obs - 1) * (1.0 + phi_c)
    ...
    print("       ⇒ **(c) は滞留を下回る**が、**1 回だけ衝突して以後凍結する方策では差が薄い**"
          "（0.005 前後）。\n"
          "          **繰り返し衝突する方策には明確に効く**（差が広がる）。")
    ...
    print("       模型（α 掃引・N=10）: **−2.0〜−7.5** ＝ **どちらにも大きく負ける** ✅")
    print("       実走 regime（T≈608 歩ごとに 9 回）: **−0.200〜−0.213** ＝ "
          "**探索に負ける** ✅・**滞留とはほぼ同着〜わずかに負ける**")
    print("       ⇒ **繰り返し衝突が「得な戦略」になることはない**が、"
          "**実走 regime では滞留との差が薄い**ので、\n"
          "          **順序の回復を (c) だけに依存させるのは薄氷**である"
          "（訪問ボーナスと時間罰の比が効いているため）。")
