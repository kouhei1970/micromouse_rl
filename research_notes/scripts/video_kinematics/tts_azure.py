...
READINGS: list[tuple[str, str]] = [
    # 🔴 読み間違いが実測で見つかったものを先頭に置く（長い表記から順に当たるため）。
    #    ユーザ指摘 2026-08-20: 「前後端」→ ぜんこうたん（ぜんごたん ではない）、
    #    「得」→ とく（える ではない）。合成した音声を必ず聞いて確かめること。
    (r"前後端",        "ぜんこうたん"),
    (r"何が得なのか",  "なにがとくなのか"),
    (r"得なのか",      "とくなのか"),
    (r"斜めは得",      "ななめはとく"),
    # ユーザ指摘 2026-08-22（センサ動画のナレーション作成時）: 「最短走行」が既定では
    # 「最短操業」（さいたんそうぎょう）に聞こえる（Azure STT で往復確認して見つかった。
    # 「探索走行」は同じ手順で確認済みで既定のまま正しい＝たんさくそうこう）。
    (r"最短走行",      "さいたんそうこう"),
    (r"η",            "イータ"),
    (r"m/s²",         "メートル毎秒毎秒"),
    (r"m/s",          "メートル毎秒"),
    ...
    (r"迷路",          "めいろ"),
    (r"区画",          "くかく"),
    # センサ動画（sensor_visualization）で使う語。2026-08-22、Azure STT で音声を文字に
    # 起こして往復確認した（「最短走行」以外は既定のままで下記の読みと一致していたが、
    # 声質の更新など将来の変化に備えて固定しておく）。
    (r"満量比",        "まんりょうひ"),
    (r"壁切れ",        "かべぎれ"),
    (r"側方センサ",    "そくほうセンサ"),
    (r"探索走行",      "たんさくそうこう"),
