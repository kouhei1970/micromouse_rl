...
def main():
    ...
    print(f"水準数 {len(S)}／安全率 {sfs}")
    if len(S) < 8:
        print("⚠️ **8 水準が揃っていない。判定は確定ではない**（中間の集計として読むこと）\n")
    ...
    print(f"\n【C1】完走率の単調非増加: {'的中' if c1 else '外れ'}"
          f"（上昇した水準 {len(ups)} 件・うち 1 迷路ぶん〔{1/n0:.3f}〕を超える上昇 {len(big_ups)} 件）")
    for sf, du in ups:
    ...
    print(f"    最速タイムを持つ迷路の数（各水準）: {ns}"
          + ("  ← **全水準で同じなら母集団は減っていない**" if len(set(ns)) == 1 else
             "  ← ⚠️ **水準によって母集団が違う**"))
