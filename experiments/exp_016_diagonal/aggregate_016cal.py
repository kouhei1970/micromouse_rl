...
--------------------------------------------------------------------------
⚠️ C3 は 2 通り出す（**生存者の選択が入るため**・§7-1）
--------------------------------------------------------------------------
「安全率ごとの最速タイム中央値」は、**失敗した面には最速タイムが無い**ので、
実際には**完走した面だけの中央値**である。**完走率が低い段では母集団が違う。**

    - **全段**（登録した条文どおり）
    - **完走率 ≥ 90% の範囲に限ったもの**（参考。母集団が揃っている範囲）
...
C5_TOP3_SHARE = 0.50       # C5: 最初に落ちる 3 面が全失敗の 50% 未満
...
def main():
    ...
    big_ups = [u for u in ups if u[1] > 1.0 / n0 + 1e-12]   # 1 面ぶんを超える上昇
    c1 = not big_ups
    print(f"\n【C1】完走率の単調非増加: {'的中' if c1 else '外れ'}"
          f"（上昇した段 {len(ups)} 件・うち 1 面ぶん〔{1/n0:.3f}〕を超える上昇 {len(big_ups)} 件）")
    for sf, du in ups:
    ...
    print("    ⚠️ **低い完走率の段は「生き残った面だけの中央値」で母集団が違う**（§7-1）")
    ...
    print(f"\n【C5】失敗の集中: 全失敗 {tot} 件・上位 3 面が {top3} 件（{share*100:.1f}%）"
          f": {'的中' if c5 else '外れ'}"
          + ("" if c5 else " → **F-4: 完走率 1 つで扱うのは不適切。面の性質で層別する**"))
    if cnt:
        print(f"    最も落ちた面: {cnt.most_common(5)}")
