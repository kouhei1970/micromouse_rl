#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""exp_013 の 4 条件を集計し、カード §5 の必須項目を出す。

**入力**（`run_arm.py` が残したもの。凍結ハーネスには触れない）:
  - `outputs/exp_013_band_v4_reeval/<条件>/runs_detail.json` … 走行ごとの
    (歩数 n_cells, 旋回数 n_turns=裁定R4, 実走距離, 平均速度, visited_cells)
  - `outputs/exp_013_band_v4_reeval/<条件>/traj/<maze>.npz`  … 走行境界
...
**指標の出所**（`docs/RESEARCH_PLAN.md` §2 を正とする）:
...
# ==========================================================================
# 1 条件の読み込み: runs_detail.json × traj の走行境界 → 面ごとの走行列
# ==========================================================================
def load_arm(arm):
...
def per_maze_metrics(maze, rec):
    ...
    #      （§2 を正とする。凍結ハーネスの first_fast_efficiency は分母が
...
# ==========================================================================
# E2 の発動面数: 条件1 と条件2 で**走行構成が違う面**を数える
...
def render(summaries, act, missing):
    ...
    if missing:
        w("> ⚠️ **未完走の条件がある**: " + " / ".join(missing) + "\n")

    w("## 1. 主要 5 指標\n")
    w("| 条件 | (a) ゴール到達率 | (b) 最短走行成立率 | (c) 有効最短走行率 | (d) 最速タイム 中央値 | **(e') 経路効率**（主） | (e) 初回最短走行効率 |")
    w("|---|---|---|---|---|---|---|")
    for s in summaries:
        ep, e = s["e_prime"], s["e"]
    ...
    w("| 条件 | 超過区画数 中央値（四分位, 最小/最大） | **真の最短を引けた面** | **負の面**（R14 の反証） | 補正前 (e') 中央値（参考・判定に使わない） |")
    w("|---|---|---|---|---|")
    for s in summaries:
        ex = s["excess"]
        if ex["n"] == 0:
    ...
    # 条件をまたいだ中央値どうしの比較は、(b) の成否で母集団が変わるため使えない
    ...
    if s1 and s2:
        ...
        # (d) は両条件とも best_time が定義される面でのみ対応をとる
        ...
        if better:
            ...
            for m, a, b in sorted(better, key=lambda t: t[1] - t[2]):
                w(f"| {m} | {a:.2f} s | {b:.2f} s | **{b-a:+.2f} s** |")
            w("")
        # ⚠️ 条件どうしの (d) 中央値の差を「E2 で悪化した」と読んではならない
        m1, m2 = s1["d"]["median"], s2["d"]["median"]
        if m1 is not None and m2 is not None:
            w(f"> ### ⚠️ (d) の**中央値どうし**を比べてはならない（本集計で実際に生じた）\n>")
            w(f"> 条件ごとの (d) 中央値は **{m1:.2f} s → {m2:.2f} s（{m2-m1:+.2f} s）**"
              f"と動くが、**対応をとった差の中央値は "
        ...
        w(f"**P3 (e) も悪化するはず**: 両条件とも (e) が**退化せず定義された**面は "
    ...
    w("- **v3 帯と v4 帯は 20 面中 10 面を共有している**"
      ...
      "さらに exp_007 の eval 条件は **v2 帯**で走っており、v4 と重なるのは **2 面のみ**である")
    w("- **`n_turns` は裁定 R4 が正とする定義**（区画列の進行方向変化。180° = 2）で記録した。"
      "±45° リセットあり定義は使っていない")
    w("- **(e') は裁定 R14 の定義（分子 = 節点数 = 移動回数 + 1）で算出した。**"
      "補正前の値（移動回数 ÷ D_true）も参考として併記してあるが**判定には使わない**。"
      "過去の報告に (e') の数値が出ている場合、**どちらの分子で計算されたものかを確認すること**")
    w("- **【裁定 2026-08-13-R15】(e) の分母は §2 を正とする**（探索走行を含む"
    ...
    w("- **L0-b・L0-c には E1 が統合されていない**ので、"
      "**E2 の効果を測れるのは条件 1 対条件 2 の対比だけ**である（カード §2）\n")
    return "\n".join(L)
