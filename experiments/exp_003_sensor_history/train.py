...
def make_env(rank: int, gamma: float, log_dir: Path, potential_offset: bool = False,
              ...
              action_highpass_penalty: float = 0.0, action_highpass_alpha: float = 0.5):
    ...
    def _init():
        env = CorridorEnv(
            ...
            action_highpass_penalty=action_highpass_penalty,
...
def main(argv=None):
    ...
    parser.add_argument("--action-highpass-penalty", type=float, default=0.0,
                        help="exp_006 案 3: 行動の高周波成分への罰の係数 k"
                             "（−k‖a_t − ā_t‖²、ā は低域通過成分）")
    parser.add_argument("--action-highpass-alpha", type=float, default=0.5,
                        help="exp_006 案 3: 低域通過の係数 α（既定 0.5 ＝ τ 14.4 ms・"
                             "遮断 11 Hz）。ā_t = α·ā_(t−1) + (1−α)·a_t")
    ...
    run_summary = dict(
        ...
        action_highpass_penalty=float(args.action_highpass_penalty),
        action_highpass_alpha=float(args.action_highpass_alpha),
