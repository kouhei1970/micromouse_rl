"""
experiments/exp_010_m2_0/train.py
=================================
M2-0（6x6 迷路の単走・オドメトリによる自己位置推定）の PPO 学習スクリプト。

`experiments/exp_003_sensor_history/train.py` を土台にし、環境を
`CorridorEnv` → `Maze6Env` へ差し替えたもの。**M1 で確定した設定はそのまま持ち込む**:

| 項目 | 値 | 根拠 |
|---|---|---|
| ポテンシャル整形 | Φ = D₀ − d（ゴールまでの迷路距離） | exp_005。滞留の局所解を潰す |
| 衝突罰 | −1.0 | exp_005。Φ オフセットが生む早期終了ボーナスを相殺 |
| 訪問報酬 | +0.02 / 未訪問区画 | `experiments/m2_design.md`。机上検算済み（下記） |
| **行動の高周波成分への罰** | **α=0.5・k=8.7e-3** | **exp_006 案 3。M1 で 3 基準を達成した構成** |
| 並列環境数 | **1** | exp_008。並列 6 は科学的には成立するが**実時間で 1.9 倍遅い** |
| γ | 0.995（ポテンシャル整形と PPO で一致させる） | 理論保証の前提 |

**M2-0 の報酬設計は机上計算のみで、学習は本スクリプトが初めて**
（`experiments/n1_evidence_audit.md` の D 項「n=0」）。D₀=1.8 m・速度 0.96 m/s・
上限 6000 ステップでの割引後総収益:
...
使い方:
    .venv/bin/python experiments/exp_010_m2_0/train.py --smoke     # 疎通確認
    .venv/bin/python experiments/exp_010_m2_0/train.py \
        --action-highpass-penalty 8.7e-3 --seed 1 \
        --log-dir logs/exp_010_m2_0_seed1 --model-out models/exp_010_m2_0_seed1.zip
"""
import argparse
import json
import os
...
def main(argv=None):
    p = argparse.ArgumentParser(description="M2-0（6x6 迷路の単走）PPO 学習")
    ...
    p.add_argument("--log-dir", type=str, default="logs/exp_010_m2_0")
    p.add_argument("--model-out", type=str, default="models/exp_010_m2_0.zip")
    ...
    if args.init_model:
        ...
        # ハイパーパラメータは M1（exp_003〜006）と同一。1 実験 1 変更を守る
    ...
    with open(log_dir / ("smoke_run_summary.json" if args.smoke else "run_summary.json"),
        ...
        json.dump(dict(
            experiment="exp_010_m2_0", smoke=bool(args.smoke), total_steps=total_steps,
