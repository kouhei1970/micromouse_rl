...
class MicromouseMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        r_center_bonus = 0.0  # 通路中央を走るボーナス
        r_heading = 0.0
        r_velocity = 0.0
        r_time = -0.03
        ...
        # Progress reward（ゴールに近づく報酬）
        r_progress = 25.0 * (self.last_distance - dist_to_goal)
        self.last_distance = dist_to_goal

        # Wall Proximity（壁2cm以下で強いペナルティ）
        if min_wall_dist < 0.02:
            r_wall_proximity = -10.0  # 強いペナルティ（擦れそう）
        elif min_wall_dist < 0.04:
            r_wall_proximity = -2.0 * (0.04 - min_wall_dist) / 0.02

        # Center Bonus（通路中央を走るボーナス）
        # 左右のセンサー値が近ければ中央を走っている
        left_sensors = sensor_distances[0:2]  # LF, LS
        right_sensors = sensor_distances[2:4]  # RF, RS
        left_avg = np.mean(left_sensors)
        right_avg = np.mean(right_sensors)
        balance = 1.0 - abs(left_avg - right_avg) / 0.15  # 0.15はセンサー最大値
        if balance > 0.7:
            r_center_bonus = 0.3 * balance

        # Heading（ゴール方向を向いている報酬）
        r_heading = 0.3 * (1.0 - heading_error / np.pi)

        # Velocity（前進報酬、壁近くで減速推奨）
        if linear_vel > 0:
            if min_wall_dist > 0.04:
                r_velocity = 0.2 * min(linear_vel, 0.5)
            else:
                # 壁に近い時は低速が良い
                r_velocity = 0.1 * min(linear_vel, 0.2)

        # Collision Penalty（衝突ペナルティ）
        if collision:
            r_collision = -30.0
        ...
        reward = r_goal + r_goal_orientation + r_intermediate + r_collision + r_progress + r_wall_proximity + r_center_bonus + r_heading + r_velocity + r_time
