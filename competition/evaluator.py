...
def aggregate_kpi(maze_results) -> dict:
    ...
    return {
        ...
        "e_first_fast_efficiency": {
            ...
            # 誤読防止の注記（教授指示 2026-08-10）: 未定義面を除いた中央値だけで
            # 優劣を語ってはいけない。(e)=1.00 でも「探索が優秀」とは限らず、
            # 「走り直しても速くならない迷路だった＝探索に意味がなかった」場合も
            # 同じ値になる（規定違反の旧迷路が実例）。必ず (c) と n_undefined と
            # 併せて読むこと
            "caveat": ("未定義面（初回の最短走行が成立しなかった面）を除いた値。"
                        "n_undefined と (c) 有効最短走行率を必ず併記して読むこと。"
                        "(e)=1.00 は探索が優秀な場合と、走り直しても速くならない"
                        "迷路だった場合の両方で生じる"),
        },
    }
