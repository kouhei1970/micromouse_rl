...
# 🔴 北向き開始が経路の約28%で頭打ちになる件の調査（2026-08-20・上記の続き）:
# 衝突地点は maze_41004・u=0.50 で s≈4.77〜4.79m、半径0.178mの弧の途中
# （`margin=0.005` で選ばれた、幾何ぎりぎりの弧）。ここで確定したこと:
#   1. **計画そのものは北向きと南向きで完全に同一**（`PathBlock.s_grid`/
#      `kappa_ref`/`v_ref` が34124点すべてビット一致。異なるのは南向きが
#      先頭に実在の180°その場旋回を挟むことだけ）。「61対62ターン」の差は
#      この1回の強制その場旋回の有無のみで、その後の経路形状は同一。
#   2. **北向き開始の計画の先頭へ人為的に360°その場旋回を挟んでも伸びない
#      （むしろ悪化: 593→232ティック、s=4.78→1.62、到達28.0%→9.5%）。**
#      「先頭に旋回が無いから追従が不安定」という単純な説は否定される。
#   3. `friction_use`（u）・`margin`（半径探索の壁との余裕）のどちらを
#      掃っても、隣接する値で合否が入れ替わる（例: u=0.19/0.21は28%止まり、
#      u=0.20/0.18は84.6%まで伸びる。margin=0.014/0.016は28%止まり、
#      0.012/0.015/0.018は38%台まで伸びる）。**南向き開始のalphaで既に見つけて
#      いた「なめらかに応答しない・孤立点は再現しない」性質と同じ signature**
#      であり、u=0.20やmargin=0.015個別の値を「直った」と扱わない
#      （本ファイルの既存の教訓のとおり）。
#   4. 弧の途中は横位置補正が効かない（下記 `_apply_wall_correction` の
#      `kappa_nonzero_now` 分岐）ため、この種の弧では横ずれが実測10〜20mm
#      （設計余裕5mmの2〜4倍）まで無補正で漂う。どちらの開始方位でこの弧に
#      当たるかは「この時点でのずれの位相」次第で、その位相は起動直後からの
#      履歴全体で決まる（＝(2)のとおり、履歴を少しでも変えると結果が変わる）。
# **結論: 北向き開始の頭打ちは、この特定の弧のマージンの薄さ（設計5mm）と
# 弧の間の横方向フィードバック欠如が生む、初期条件への鋭敏な依存性
# （カオス的）が原因であり、u・margin・その場旋回のどれを動かしても
# 再現性のある形では直せなかった。次に試すべきは、弧の途中でも横位置を
# 推定できる別の手段（現行の`estimate_lateral_offset`は直線前提）か、
# 半径探索のmarginを実測誤差から系統的に作り直すこと（複数迷路・複数
# 開始方位で検証してから採用する）。**
#
# 一方、この調査の副産物として `_apply_wall_correction` の弧突入時の
# `apply_lateral_correction(0.0)`（直前の直線区間の推定値を理由なく
# 0へ捨てていた）は削除した。実測（maze_41004・u=0.50/0.30）: 南向き開始
# 38.5%→47.9%に改善・北向き開始28.0%→28.0%で不変（悪化しない）。
# こちらは「値を捨てない」という一方向の設計変更であり、上記(3)のような
# 孤立点探索ではない。
...
class ClassicExplorer:
    ...
    def _apply_wall_correction(self, obs) -> None:
        ...
        **新規の推定は直線区間でのみ**行う（弧の途中は姿勢が斜めで、
        `estimate_lateral_offset` の前提 — センサが区画の壁とほぼ平行な
        向きを見ている — が成り立たないため。任務指示 2026-08-20）。その場旋回
        （`SpinSegment`）中も同じ理由で行わない。弧の途中・その場旋回中は
        横位置補正の**推定を更新しない**だけで、直前の直線区間で得ていた
        推定値は破棄しない（是正 2026-08-20。下記 (a) 参照）。
        ...
        if kappa_nonzero_now:
            # 弧の途中: 新規の推定はしない（`estimate_lateral_offset` の前提が
            # 崩れるため。docstring参照）。ただし直前の直線区間で得ていた
            # 推定値は破棄せず持ち越す（是正 2026-08-20。理由・実測は
            # モジュール冒頭 `DEFAULT_WALL_CORRECTION_LOOKAHEAD_M` 直下の
            # 「北向き開始が頭打ちになる件の調査」コメント参照）。
            return
