...
検査項目（design.md §4 末尾の (a)〜(e) ＋ (b-2) ＋ (b-3)。裁定 R8・R12 を反映）:
  ...
  (b)   跳びが無いこと。閾値は**乗法項＋加法項**の形で定義する（裁定 R7-6・R18-2・R30）:
            |ΔΦ_t| ≤ √2 · 1.10 · |Δp_t| + μ_max,  μ_max = (s − t_w)/2 − w_lat ≈ 0.0440 m
        √2 は区画内の Lipschitz 定数、μ_max は**中点一致の破れ**（速度に依存しない
        加法的な跳び）。**乗法項だけの形では低速時に比が発散して原理的に通らない。**
...
#   JUMP_L_E   … **判定には使わない**（記録として残す）。「**辺全体**での一致を課した
#                ときに必要な Lipschitz 定数の下限」であり、本設計は辺全体一致を課さない。
#                無次元形は（裁定 R24-3・R25）
...
#                課さないので辺全体一致の下限に縛られない）。
#                🔴 **乗法項として判定に使うのは誤りだった**（裁定 R30 で是正）。中点一致の
#                破れは**速度に依存しない加法的な跳び**なので、低速時に比が発散する
#                （実測: 急停止台本で比 4.385・|Δp|=0.00931）。加法項 μ_max として登録する。
#                裁定 R21-E・R24-3・R25・R30
...
# 中点一致の破れの上界 μ_max = (s − t_w)/2 − w_lat（機体中心が開口部の中点から
# 横に外れられる量）。条件 E の Φ は開口部の**中点でしか**両側が一致しないので、
# 中点から外れて区画を跨ぐと最大でこれだけ跳ぶ。**速度に依存しない加法的な量**。
...
def jump_threshold(disp: float) -> float:
    ...
    - μ_max: **中点一致の破れ**。開口部の中点から外れて跨ぐと生じる跳びの上界

    🔴 **乗法項だけの形（旧 L_E = 2.7669 を掛ける形）では原理的に通らない。**
    中点一致の破れは**速度に依存しない加法的な跳び**なので、低速時（|Δp| → 0）に
    比 |ΔΦ|/|Δp| が発散する。実測でも 急停止台本の低速域で比 4.385 が出た
    （|ΔΦ|=0.04084・|Δp|=0.00931）。**加法項として登録するのが正しい形**であり、
    条件 C の判定形（導出上界 ＋ 刻み収束）と**同形**になる（裁定 R30・D3-8:
    E と C の合格線は同じ形でなければ、差が「Φ の性質の差」か「基準の差」か
    分離できない）。

    なお L_E = s/(√2·(t_w/2 + w_lat)) = 2.7669 は「**辺全体**での一致を課したときに
    ...
    if violations:
        ...
        for v in violations:
            detail.append(
                ...
                f"（|Δp|={v['disp']:.5f}、超過 "
                f"{v['dphi'] - LIPSCHITZ_DEF * JUMP_KAPPA * v['disp']:+.5f} m／許容 "
                f"{MU_MAX:.4f}）区画 {v['cell_from']}→{v['cell_to']} "
