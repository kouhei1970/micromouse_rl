...
class MicromouseEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, xml_file="micromouse_maze.xml", max_steps=4000):
        ...
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Model file {model_path} not found. Please run maze_generator.py first.")
    ...
    def _update_targets(self):
        ...
        if r < 0.3:
            ...
            self.target_linear_velocity = 0.0
            self.target_angular_velocity = self.np_random.uniform(-2*np.pi, 2*np.pi)
        elif r < 0.6:
            # Mode 2: Straight (Angular = 0)
            # Target range updated to [0.5, 2.5] m/s as requested
            self.target_linear_velocity = self.np_random.uniform(0.5, 2.5) 
            self.target_angular_velocity = 0.0
        elif r < 0.7:
            # Mode 3: Stop (Both = 0)
            self.target_linear_velocity = 0.0
    ...
    def _render_frame(self):
        if self.render_mode == "human":
            ...
            if self.renderer is None:
                # Increase resolution to 960x960 (2x default approx)
                self.renderer = mujoco.Renderer(self.model, height=960, width=960)
